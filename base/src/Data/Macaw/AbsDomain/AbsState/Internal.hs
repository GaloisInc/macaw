...
-- "Data.Macaw.AbsDomain.AbsState".  It is exposed only so that the test
...
-- "Data.Macaw.AbsDomain.AbsState" instead.
module Data.Macaw.AbsDomain.AbsState.Internal
  ( AbsBlockState
  , setAbsIP
  , absRegState
...
joinAbsValue' v v'
    ...
    | StridedInterval si <- v,  FinSet s <- v' =
      let si_new = SI.fromFoldable (SI.typ si) s
      in if si_new `SI.isSubsetOf` si
           then return Nothing
           else return $ go si si_new
    | StridedInterval si <- v,  CodePointers s b <- v' = do
      addWords s
      let (wordSet, _) = partitionAbsoluteAddrs s b
          si_new = SI.fromFoldable (SI.typ si) wordSet
      return $ if si_new `SI.isSubsetOf` si then Nothing else go si si_new
