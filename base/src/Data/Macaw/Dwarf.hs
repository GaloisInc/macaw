...
runDIEParser ctx d act =
  runInContext (taggedError (ctx ++ " " ++ show (dieId d) ++ " " ++ show (dieTag d))) $ do
    ...
    do
      let missingTags = Map.keysSet childMap `Set.difference` (s1 ^. dpsSeenChildren)
    ...
    do
      let missingAttrs = Map.keysSet attrMap `Set.difference` (s1 ^. dpsSeenAttributes)
...
attributeAsFile fileVec _ (DW_ATVAL_UINT i) =
  if i == 0
    then pure ""
      ...
      if toInteger i <= toInteger (V.length fileVec)
        then pure $! fileVec V.! fromIntegral (i -1)
        else throwError $ InvalidFileIndex i
...
instance Pretty SubprogramDef where
  pretty d =
    vcat
      [ "low_pc:     " <+> pretty (maybe "UNDEF" (`showHex` "") (subLowPC d)),
...
instance Pretty CompileUnit where
  pretty cu =
    vcat
      [ "producer:    " <+> pretty (BSC.unpack (cuProducer cu)),
...
lookupSubprogram r cu = Map.lookup r (cuSubprogramMap cu)
...
getAddressRangeTable end enc bs = parseGet bs (go [])
  where
    readAddress = desrGetOffset end enc
    go prev = do
