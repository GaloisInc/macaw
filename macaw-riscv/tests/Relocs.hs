...
tests = do
  riscvBins <- concat <$> traverse SFG.glob
    [ "tests/riscv/*.exe"
    , "tests/riscv/*.o"
    ]
  symBins <- concat <$> traverse SFG.glob
    [ "../macaw-riscv-symbolic/tests/pass/**/*.exe"
    , "../macaw-riscv-symbolic/tests/fail/**/*.exe"
    ]
  let testCases = map (\path -> (path, path)) (riscvBins ++ symBins)
               ++ [ ( "../deps/elf-edit/tests/relocs/riscv/relocs-rv32gc.o"
                    , "tests/riscv/relocs-rv32gc.o"
                    )
                  , ( "../deps/elf-edit/tests/relocs/riscv/relocs-rv64gc.o"
                    , "tests/riscv/relocs-rv64gc.o"
                    )
                  ]
