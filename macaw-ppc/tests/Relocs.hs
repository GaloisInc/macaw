...
tests = do
  ppcBins <- concat <$> traverse SFG.glob
    [ "tests/ppc32/*.exe"
    , "tests/ppc32/*.o"
    , "tests/ppc64/*.exe"
    , "tests/ppc64/*.o"
  ...
  symBins <- concat <$> traverse SFG.glob
    [ "../macaw-ppc-symbolic/tests/pass/**/*.exe"
    , "../macaw-ppc-symbolic/tests/fail/**/*.exe"
    ]
  let testCases = map (\path -> (path, path)) (ppcBins ++ symBins)
               ++ [ ( "../deps/elf-edit/tests/relocs/ppc32/relocs.o"
                    , "tests/ppc32/relocs.o"
                    )
                  , ( "../deps/elf-edit/tests/relocs/ppc64/relocs.o"
                    , "tests/ppc64/relocs.o"
                    )
                  ]
