...
-- | Items in this module should /not/ be considered part of macaw-symbolic's
...
-- pass the correct flags to GHC for the @lib:macaw-symbolic@ target. We verify
-- that we have done so by asserting as much in the test suite.
--
-- See also:
--
-- * @cabal.project.werror@ for the @-fno-ignore-asserts@ flag
-- * @Data.Macaw.Symbolic.Memory.Lazy@ for uses of 'X.assert'
