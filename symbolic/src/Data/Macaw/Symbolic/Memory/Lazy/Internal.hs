...
data MemPtrTable sym w = MemPtrTable
  { memMutableTable :: MSMC.MutableIntervalMap (MC.MemWord w) (MemChunkBytes sym)
  ...
  , memImmutableTable :: MSMC.ImmutableIntervalMap (MC.MemWord w) (MemChunkBytes sym)
...
-- | The number of bytes in a 'MemChunkBytes'.
...
sliceContiguousChunks cache addr numBytes mutChunks immutChunks memContents populatedChunks
  ...
  where
    ...
    popNext ::
      [(IMI.Interval a, MemChunkBytes sym)] ->
      [(IMI.Interval a, MemChunkBytes sym)] ->
      Maybe (IMI.Interval a, MemChunkBytes sym,
             [(IMI.Interval a, MemChunkBytes sym)],
             [(IMI.Interval a, MemChunkBytes sym)])
    popNext = \cases
      [] [] -> Nothing
...
mergedMemoryMemChunkBytes ::
  ...
  t (MC.Memory w) ->
  m ( MSMC.MutableIntervalMap (MC.MemWord w) (MemChunkBytes sym)
    , MSMC.ImmutableIntervalMap (MC.MemWord w) (MemChunkBytes sym)
    )
...
concreteUnmutatedGlobalRead ::
  ...
  sym ->
  MSMC.MutableIntervalMap (MC.MemWord w) (MemChunkBytes sym) ->
  MSMC.ImmutableIntervalMap (MC.MemWord w) (MemChunkBytes sym) ->
  MBC.ByteCache sym ->
